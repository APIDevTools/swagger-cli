...
module.exports = function dereference(filename, options, cb) {
  var output = [];
  output.push(util.format('Bundling file: %s', filename));

  var dereferenceInternal$Refs = options.dereference ? true : false;

  swagger.parse(filename, {dereferenceInternal$Refs: dereferenceInternal$Refs}, function(err, api, metadata) {
